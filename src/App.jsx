...
export default function App() {
  ...
  const processPassRound = () => {
    ...
    if (charlestonStep < 5) {
      const nextStep = charPassSteps[charlestonStep + 1];
      setCharlestonStep(s => s + 1);
  ...
  const handleDraw = () => {
    ...
    const dCopy = [...deck];
    if (dCopy.length === 0) { setGameState('finished'); return; }
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-900 font-sans text-slate-100 overflow-hidden">
      ...
      {showWinDeclare && (
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-slate-800 w-full max-w-2xl max-h-[70vh] rounded-[3rem] border-2 border-slate-700 overflow-hidden flex flex-col shadow-2xl animate-in zoom-in">
